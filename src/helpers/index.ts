...
function compilePath(path: string): [RegExp, string[]] {
    const paramNames = [];
...
// TODO helper should be removed after updating react-router library
export const matchPath = (pattern: string, pathname: string) => {
    ...
    let pathnameBase = matchedPathname.replace(PATH_NAME_REG_EXP, '$1');
    const captureGroups = match.slice(1);
    const params = paramNames.reduce<Record<string, string>>((memo, paramName, index) => {
        if (paramName === '*') {
            const splatValue = captureGroups[index] || '';
            pathnameBase = matchedPathname.slice(0, matchedPathname.length - splatValue.length).replace(PATH_NAME_REG_EXP, '$1');
        }
...
export type BreadcrumbMatch = any;
...
export type BreadcrumbsRoute = any;
...
const getDefaultBreadcrumb = ({
    ...
    const match = matchPath(pathSection, pathSection)!;
...
const getBreadcrumbMatch = ({
    ...
    const getIsPathExcluded = (path: string): boolean => matchPath(pathSection, path) != null;
